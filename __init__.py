...
bl_info = {
    'name': 'glTF 2.0 format for Foundation',
    ...
    'author': "mos9527, Blender Foundation, Khronos Group",
    "version": (5, 2, 23),
    'blender': (5, 2, 0),
    'location': 'File > Import-Export',
    'description': 'Import-Export as glTF 2.0 for Foundation',
    'warning': '',
    'tracker_url': "https://github.com/mos9527/Foundation-Blender-IO/issues/",
    'category': 'Import-Export',
}


def _addon_package():
    """Return the top-level addon package name (e.g. 'io_scene_foundation').

    This addon is designed to co-exist with the official ``io_scene_gltf2``
    addon; every place that used to hard-code ``'io_scene_gltf2'`` must look
    up its own package instead so preferences, module paths, etc. do not
    collide with the official implementation.
    """
    # __package__ for submodules is e.g. 'io_scene_foundation.blender.exp';
    # for the addon's own __init__.py it is 'io_scene_foundation'. Either
    # way the first dotted component is the addon name Blender registers.
...
def on_export_format_changed(self, context):
    ...
    if not sfile.active_operator:
        return
    if sfile.active_operator.bl_idname != "EXPORT_SCENE_OT_gltf_foundation":
        return
...
def get_format_items(scene, context):
    ...
    addon_preferences = bpy.context.preferences.addons[_addon_package()].preferences
...
class ExportGLTF2_Base(ConvertGLTF2_Base):
    ...
    export_lights: BoolProperty(
        name='Lights',
        description='Export punctual lights (directional, point, spot) via '
                    '"KHR_lights_punctual", and area lights (rectangle, disk) '
    ...
    def draw(self, context):
        ...
        gltfpack_path = context.preferences.addons[_addon_package()].preferences.gltfpack_path_ui.strip()
        if gltfpack_path != '':
...
class ExportGLTF2(bpy.types.Operator, ExportGLTF2_Base, ExportHelper):
    """Export scene as glTF 2.0 file (Foundation)"""
    bl_idname = 'export_scene.gltf_foundation'
    bl_label = 'Export glTF 2.0 (Foundation)'
...
def menu_func_export(self, context):
    self.layout.operator(ExportGLTF2.bl_idname, text='glTF 2.0 (.glb/.gltf) (Foundation)')


class ImportGLTF2(Operator, ConvertGLTF2_Base, ImportHelper):
    """Load a glTF 2.0 file (Foundation)"""
    bl_idname = 'import_scene.gltf_foundation'
    bl_label = 'Import glTF 2.0 (Foundation)'
...
class IO_FH_gltf2_foundation(bpy.types.FileHandler):
    bl_idname = "IO_FH_gltf2_foundation"
    bl_label = "glTF 2.0 (Foundation)"
    bl_import_operator = "import_scene.gltf_foundation"
    bl_export_operator = "export_scene.gltf_foundation"
...
def register():
    ...
    if bpy.context.preferences.addons[_addon_package()].preferences.animation_ui is True:
