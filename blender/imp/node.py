...
class BlenderNode():
    ...
    def create_object(gltf, vnode_id):
        ...
        if vnode.mesh_node_idx is not None:
            ...
            ext = pynode.extensions or {}
            if 'KHR_lights_punctual' in ext:
                light = BlenderLight.create(gltf, vnode, ext['KHR_lights_punctual']['light'])
            else:
                # EXT_lights_area — the vnode selector already guaranteed one of
                # the two extensions is present.
                light = BlenderLightArea.create(gltf, vnode, ext['EXT_lights_area']['light'])
