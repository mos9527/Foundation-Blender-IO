...
from . import lights_area as gltf2_blender_gather_lights_area
...
def __gather_extensions(vnode, export_settings):
    ...
    if blender_lamp is not None:
        ...
        if light is not None:
            ...
            # Not a punctual light (e.g. Blender AREA); try EXT_lights_area.
            area_light = gltf2_blender_gather_lights_area.gather_lights_area(
                blender_lamp,
                world_matrix,
                export_settings,
            )
            if area_light is not None:
                area_light_extension = gltf2_io_extensions.ChildOfRootExtension(
                    name="EXT_lights_area",
                    path=["lights"],
                    extension=area_light,
                )
                extensions["EXT_lights_area"] = gltf2_io_extensions.Extension(
                    name="EXT_lights_area",
                    extension={
                        "light": area_light_extension,
                    },
                )
                if len(export_settings['current_paths']) > 0:
                    # Reuse the shared `lights` animation-pointer dict: the
                    # sampling/keyframe code dispatches on the path string, so
                    # KHR_lights_punctual and EXT_lights_area entries coexist
                    # without collision (keyed by id(blender_lamp)).
                    export_settings['KHR_animation_pointer']['lights'][id(blender_lamp)] = {}
                    export_settings['KHR_animation_pointer']['lights'][id(
                        blender_lamp)]['paths'] = export_settings['current_paths'].copy()
                    export_settings['KHR_animation_pointer']['lights'][id(
                        blender_lamp)]['glTF_light'] = area_light_extension
