...
def __filter_lights_punctual(blender_lamp, export_settings) -> bool:
    if blender_lamp.type == "HEMI":
        export_settings['log'].warning("Unsupported light source {}".format(blender_lamp.type))
        return False
    if blender_lamp.type == "AREA":
        # Area lamps are handled by the EXT_lights_area extension; skip silently here.
        return False
...
def __gather_color(blender_lamp, export_settings) -> Optional[List[float]]:
    return gather_lamp_color(
        blender_lamp,
        "/extensions/KHR_lights_punctual/lights/XXX/color",
        export_settings,
    )


def gather_lamp_color(blender_lamp, animation_pointer_path, export_settings) -> Optional[List[float]]:
    """Shared color-gathering logic used by both KHR_lights_punctual and EXT_lights_area.

    `animation_pointer_path` is the glTF JSON pointer template used to register
    the KHR_animation_pointer entry, e.g.
    "/extensions/KHR_lights_punctual/lights/XXX/color" or
    ...
    if emission_node is not None:
        ...
        path_ = {}
        path_['length'] = 3
        path_['path'] = animation_pointer_path
