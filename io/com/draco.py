...
def dll_path() -> Path:
    ...
    # Resolve addon root via this module's own package (two levels up from
    # io.com.draco). This keeps the Draco lib lookup pointing at *our* addon
    # even when the official ``io_scene_gltf2`` is also installed.
    addon_package = __package__.split('.')[0]
    path = os.path.dirname(sys.modules[addon_package].__file__)
    if path is not None:
